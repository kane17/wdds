...
def get_amount_of_cities_with_more_than_one_house():
    sqlquery = "SELECT DISTINCT c.City FROM City c LEFT JOIN Address a ON a.FK_City = c.ID LEFT JOIN County co ON co.ID = a.FK_County WHERE ((SELECT COUNT(DISTINCT Address.FK_County) FROM Address WHERE Address.FK_City=c.ID) > 1);"
